...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn('Backend health check failed:', (healthError as Error).message);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (data.error) {
          errorMessage = typeof data.error === 'string' ? data.error : data.error.message || 'Chat failed';
        } else if (data.detail) {
        ...
        if (response.status === 500) {
          errorMessage = 'AI 분석 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.';
        } else if (response.status === 404) {
          errorMessage = '요청한 데이터셋을 찾을 수 없습니다. 데이터 전처리가 완료되었는지 확인해주세요.';
    ...
    return NextResponse.json(
      { error: 'AI 서버와의 연결에 문제가 발생했습니다. 잠시 후 다시 시도해주세요.' },
      { status: 500 }
    );
  }
}